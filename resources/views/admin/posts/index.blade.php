                            ...
                            <table class="table">
                                ...
                                @foreach($posts as $post)
                                    <tr>
                                        ...
                                        <td class="d-flex">
                                            @if(request()->input('status') != \App\Models\Post::STATUS_PENDING)
                                                <form
                                                    method="post"
                                                    action="{!! route('admin.posts.make', ['post' => $post, 'status' => \App\Models\Post::STATUS_PENDING]) !!}">
                                                    @method('put')
                                                    @csrf
                                            ...
                                            @endif
                                            @if(request()->input('status') != \App\Models\Post::STATUS_ACCEPTED)
                                                <form
                                                    method="post"
                                                    class="ml-1"
                                                    action="{!! route('admin.posts.make', ['post' => $post, 'status' => \App\Models\Post::STATUS_ACCEPTED]) !!}">
                                                    @method('put')
                                                    @csrf
                                            ...
                                            @endif
                                            @if(request()->input('status') != \App\Models\Post::STATUS_REJECTED)
                                                <form
                                                    method="post"
                                                    class="ml-1"
                                                    action="{!! route('admin.posts.make', ['post' => $post, 'status' => \App\Models\Post::STATUS_REJECTED]) !!}">
                                                    @method('put')
                                                    @csrf
